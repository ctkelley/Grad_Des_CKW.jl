function Figure1_2a(n = 15; maxit = 20000, alpha = 0.5)
    ...
    ts=L"\tau_0"
    tautex="testing "*ts*". n=$n, alpha=$alpha"
    println(tautex)
...
function Figure1_2b(n = 15; maxit = 20000, tau0 = 0.01)
    (n > 15) && (maxit=100000)
    ts=L"\tau_0"
    ptex="Alg 1; testing alpha. n=$n," * ts * "=$tau0"
...
function Figure3(n = 15; maxit = 20000, tau0 = 0.1)
    (n > 15) && (maxit=100000)
    ts=L"\tau_0"
    ptex="Alg 2; testing alpha. n=$n," * ts * "=$tau0"
    p_test1(n; maxit = maxit, algfun = alg2e1)
    title(ptex)
    #title("Alg 2; testing p. n=$n, tau0=$tau0")
end

function Figure4_5a(n = 15; maxit = 20000, p = 0.5, tauvec = [0.2, 0.1, 0.05, 0.01])
    (n > 15) && (maxit=100000)
    tau_test1(n; maxit = maxit, algfun = alg3e1, tauvec = tauvec)
    ts=L"\tau_0"
    tautex="testing "*ts*". n=$n, alpha=$p"
    println(tautex)
    #title("Alg 3; testing tau. n=$n, alpha=$p")
    title(tautex)
end

function Figure4_5b(n = 15; maxit = 20000, tau0 = 0.01)
    (n > 15) && (maxit=100000)
    ts=L"\tau_0"
...
function Figure6ab(n = 15; maxit = 1000, tau0 = 20.0, p = 0.5)
    ts=L"\tau_0"
