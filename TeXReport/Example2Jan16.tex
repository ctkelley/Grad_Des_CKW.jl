...
\begin{equation}
	\label{example2}
	0 = \bfF (\bfu) := \dfrac{1}{\theta} \dkh{ \bfu - \proj_{\bfU} \dkh{ \bfu - \theta \dkh{ \bfA \bfu + \delta \abs{\bfu}^{\alpha} \sign (\bfu) - \abs{\bfu}^{p - 1} \bfu - \bfb } } },
\end{equation}
...
As was the case for \cref{subsec:example}, problems where the exponent $\alpha$ for the non-Lipschitz term in the gradients is small are difficult. 
...
\begin{figure}[h!]
    ...
    \caption{Numerical performance of \cref{alg:gd} and \cref{alg:ufgm} for problem~\cref{opt:test2} with smaller values of $\alpha$.}
    \label{fig:ex2t1}
\end{figure}
