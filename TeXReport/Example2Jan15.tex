...
\begin{equation}
	\label{example2}
	0 = \bfF (\bfu) := \bfu - \proj_{\bfU} \dkh{ \bfu - \theta \dkh{ \bfA \bfu + \delta \abs{\bfu}^{\alpha} \sign (\bfu) - \abs{\bfu}^{p - 1} \bfu - \bfb } },
\end{equation}
where $\bfU = [-1, 1]^n$,  $\theta > 0$ is a constant, $\bfA \in \Rnn$ is a symmetric positive definite matrix, and $\bfb \in \Rn$ encodes the boundary conditions. 
...
In this example we do not have an analytic solution, so we only plot the residual norms $\norm{ \bfF (\bfu) }$. 
We compare the performance of \cref{alg:gd} and \cref{alg:ufgm} on problem~\eqref{opt:test2}. 
The stepsizes of \cref{alg:gd} and \cref{alg:ufgm} are set to $0.1 h^2$ and $20 h^2$, respectively. 
In our examples, we vary $\alpha$ over the values in $\{0.1, 0.2, 0.5, 0.8\}$, while fixing $p = 1.5$ and $\delta = 20$. 
The numerical results are provided in \Cref{fig:ex2}. 
%It can be observed that both algorithms demonstrate strong performance. 
%Furthermore, 
It can be observed that, \cref{alg:ufgm} exhibits a faster convergence rate, benefiting from the use of a larger stepsize. 
