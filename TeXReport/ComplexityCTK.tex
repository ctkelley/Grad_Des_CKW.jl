...
Moreover, in \cite{Chen2025new} we show that $\nabla f$ is locally H\"older continuous, but not globally H\"older continuous.
In fact, from
...
On the other hand, this example satisfies \cref{asp:function} (ii) as
$$|\nabla f_1(x)-\nabla f_1(y)|\le L_1|x-y| \quad {\rm and} \quad |\nabla f_2(x)-\nabla f_2(y)|\le L_2|x-y|^{1/2}, \,\, \forall \, x,y \in \mathbb{R}$$
with $L_1=L_2=1$.
\end{example}
...
For example, when $\hat{\alpha} = 1$, our bound is  $O(\log(\varepsilon^{-1}))$ but (51) in \cite{Devolder2014first} is $O(\varepsilon^{-2})$.
...
\begin{lemma}
	Let $\{\sigma_k\}$ and $\{\phi_k\}$ be the sequences defined in \cref{le:phi}.
	Then, for the sequence $\{\bfu_k\}$ generated by \cref{alg:ufgm}, it holds that
	\begin{equation}
		\label{eq:descent}
