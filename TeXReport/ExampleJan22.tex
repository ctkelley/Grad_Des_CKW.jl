...
\subsubsection{Numerical results for \cref{alg:gd}}
...
In the second experiment, we vary the H{\"o}lder exponent $\alpha$ over the values in $\{0.1, 0.2, 0.5, 0.8\}$, while fixing $\tau_0 = 0.1$.
...
In \Cref{subfig:alpha}, and in several others, the thick lines are artifacts of small rapid oscillations in the error norm which reflect stagnation in the iteration. 
...
\subsubsection{Numerical results for \cref{alg:upgm}}
...
The stepsize is initialized to $20 h^2$ in the line-search procedure.
The corresponding numerical results are depicted in \cref{fig:alpha3}.
Comparing \cref{fig:alpha3} to \cref{subfig:alpha} shows the benefits of the line-search procedure in \cref{alg:upgm}, which does not need to manually adjust the value of $\tau_0$ to converge for a given value of $\varepsilon$.
...
\subsubsection{Numerical results for \cref{alg:ufgm}}
...
We first use the values for $\tau_0$ from \cref{fig:gd}.
...
Consequently, we consider larger values for $\tau_0$ in \cref{subfig:stepsize4} and set $\tau_0 = 20$ in \cref{subfig:alpha4}.
